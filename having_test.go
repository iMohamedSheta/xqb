package xqb_test

import (
	"testing"

	"github.com/iMohamedSheta/xqb"
	"github.com/stretchr/testify/assert"
)

func Test_Having_WithRawExpressions(t *testing.T) {
	qb := xqb.Table("orders")
	sql, bindings, _ := qb.Select("user_id", xqb.Raw("SUM(amount) as total")).
		GroupBy("user_id").
		Having(xqb.Raw("SUM(amount)"), ">", 1000).
		ToSQL()

	expected := "SELECT user_id, SUM(amount) as total FROM orders GROUP BY user_id HAVING SUM(amount) > ?"
	assert.Equal(t, expected, sql)
	assert.Equal(t, []any{1000}, bindings)
}

func Test_Having_Simple(t *testing.T) {
	qb := xqb.Table("orders")
	sql, bindings, _ := qb.Select("user_id", xqb.Raw("SUM(amount) as total")).
		GroupBy("user_id").
		Having("SUM(amount)", ">", 500).
		ToSQL()

	expected := "SELECT user_id, SUM(amount) as total FROM orders GROUP BY user_id HAVING SUM(amount) > ?"
	assert.Equal(t, expected, sql)
	assert.Equal(t, []any{500}, bindings)
}

func Test_Having_Raw(t *testing.T) {
	qb := xqb.Table("orders")
	sql, bindings, _ := qb.Select("user_id", xqb.Raw("SUM(amount) as total")).
		GroupBy("user_id").
		HavingRaw("SUM(amount) > ?", 1000).
		ToSQL()

	expected := "SELECT user_id, SUM(amount) as total FROM orders GROUP BY user_id HAVING SUM(amount) > ?"
	assert.Equal(t, expected, sql)
	assert.Equal(t, []any{1000}, bindings)
}

func Test_OrHaving_WithExpressions(t *testing.T) {
	qb := xqb.Table("orders")
	sql, bindings, _ := qb.Select("user_id", xqb.Raw("SUM(amount) as total")).
		GroupBy("user_id").
		Having("SUM(amount)", ">", 1000).
		OrHaving("SUM(discount)", ">", 200).
		ToSQL()

	expected := "SELECT user_id, SUM(amount) as total FROM orders GROUP BY user_id HAVING SUM(amount) > ? OR SUM(discount) > ?"
	assert.Equal(t, expected, sql)
	assert.Equal(t, []any{1000, 200}, bindings)
}

func Test_OrHaving_WithRaw(t *testing.T) {
	qb := xqb.Table("orders")
	sql, bindings, _ := qb.Select("user_id", xqb.Raw("SUM(amount) as total")).
		GroupBy("user_id").
		HavingRaw("SUM(amount) > ?", 1000).
		OrHavingRaw("SUM(discount) > ?", 200).
		ToSQL()

	expected := "SELECT user_id, SUM(amount) as total FROM orders GROUP BY user_id HAVING SUM(amount) > ? OR SUM(discount) > ?"
	assert.Equal(t, expected, sql)
	assert.Equal(t, []any{1000, 200}, bindings)
}

func Test_Having_WithMultiple(t *testing.T) {
	qb := xqb.Table("orders")
	sql, bindings, _ := qb.Select("user_id", xqb.Raw("SUM(amount) as total")).
		GroupBy("user_id").
		Having("SUM(amount)", ">", 1000).
		Having("SUM(discount)", "<", 100).
		ToSQL()

	expected := "SELECT user_id, SUM(amount) as total FROM orders GROUP BY user_id HAVING SUM(amount) > ? AND SUM(discount) < ?"
	assert.Equal(t, expected, sql)
	assert.Equal(t, []any{1000, 100}, bindings)
}

func Test_Having_WithExpressionToExpression(t *testing.T) {
	expr1 := xqb.Raw("SUM(amount)")
	expr2 := xqb.Raw("SUM(discount)")
	qb := xqb.Table("orders")

	sql, bindings, _ := qb.Select("user_id", xqb.Raw("SUM(amount) as total")).
		GroupBy("user_id").
		Having(expr1, ">", expr2).
		ToSQL()

	expected := "SELECT user_id, SUM(amount) as total FROM orders GROUP BY user_id HAVING SUM(amount) > SUM(discount)"
	assert.Equal(t, expected, sql)
	assert.Empty(t, bindings)
}

func Test_Having_WithIsNull(t *testing.T) {
	qb := xqb.Table("orders")
	sql, bindings, _ := qb.Select("user_id", xqb.Raw("COUNT(*) as cnt")).
		GroupBy("user_id").
		Having("COUNT(*)", "IS NULL", nil).
		ToSQL()

	expected := "SELECT user_id, COUNT(*) as cnt FROM orders GROUP BY user_id HAVING COUNT(*) IS NULL"
	assert.Equal(t, expected, sql)
	assert.Empty(t, bindings)
}

func Test_Having_WithExpressionValue(t *testing.T) {
	qb := xqb.Table("orders")
	sql, bindings, _ := qb.Select("user_id").
		GroupBy("user_id").
		Having(xqb.Raw("SUM(amount)"), ">", xqb.Raw("AVG(amount)")).
		ToSQL()

	expected := "SELECT user_id FROM orders GROUP BY user_id HAVING SUM(amount) > AVG(amount)"
	assert.Equal(t, expected, sql)
	assert.Empty(t, bindings)
}

func Test_Having_WithExpressionAndBoundValue(t *testing.T) {
	qb := xqb.Table("orders")
	sql, bindings, _ := qb.Select("user_id").
		GroupBy("user_id").
		Having(xqb.Raw("SUM(amount)"), ">", 100).
		ToSQL()

	expected := "SELECT user_id FROM orders GROUP BY user_id HAVING SUM(amount) > ?"
	assert.Equal(t, expected, sql)
	assert.Equal(t, []any{100}, bindings)
}

func Test_Having_WithExpressionValueAndBindings(t *testing.T) {
	qb := xqb.Table("orders")
	sql, bindings, _ := qb.Select("user_id").
		GroupBy("user_id").
		Having(xqb.Raw("COALESCE(SUM(amount), ?)", 15), "=", xqb.Raw("?", 25)).
		ToSQL()

	expected := "SELECT user_id FROM orders GROUP BY user_id HAVING COALESCE(SUM(amount), ?) = ?"
	assert.Equal(t, expected, sql)
	assert.Equal(t, []any{15, 25}, bindings)
}
