...
func Test_Join_String_Table(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		qb := xqb.Table("users").SetDialect(dialect).Join("posts", "users.id = posts.user_id")
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users JOIN posts ON users.id = posts.user_id",
			dialects.DriverPostgres: "SELECT * FROM users JOIN posts ON users.id = posts.user_id",
		}
...
func Test_Join_With_Bindings(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users JOIN posts ON users.id = posts.user_id AND posts.status = ?",
			dialects.DriverPostgres: "SELECT * FROM users JOIN posts ON users.id = posts.user_id AND posts.status = $1",
		}
		assert.Equal(t, expectedSql[dialect], sql)
...
func Test_LeftJoin(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users LEFT JOIN comments ON users.id = comments.user_id",
			dialects.DriverPostgres: "SELECT * FROM users LEFT JOIN comments ON users.id = comments.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Empty(t, bindings)
...
func Test_RightJoin(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users RIGHT JOIN logins ON users.id = logins.user_id",
			dialects.DriverPostgres: "SELECT * FROM users RIGHT JOIN logins ON users.id = logins.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Empty(t, bindings)
...
func Test_FullJoin(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "",
			dialects.DriverPostgres: "SELECT * FROM users FULL JOIN sessions ON users.id = sessions.user_id",
		}
		expectedErr := map[dialects.Driver]error{
			dialects.DriverMySQL:    xqbErr.ErrUnsupportedFeature, // MySQL does not support FULL JOIN
			dialects.DriverPostgres: nil,
		}
...
func Test_CrossJoin(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users CROSS JOIN roles",
			dialects.DriverPostgres: "SELECT * FROM users CROSS JOIN roles",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Empty(t, bindings)
...
func Test_CrossJoinSub(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users CROSS JOIN (SELECT * FROM plans WHERE expired = ?) AS p",
			dialects.DriverPostgres: "SELECT * FROM users CROSS JOIN (SELECT * FROM plans WHERE expired = $1) AS p",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_CrossJoin_With_Expr(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users CROSS JOIN (SELECT * FROM regions WHERE active = ?) AS r",
			dialects.DriverPostgres: "SELECT * FROM users CROSS JOIN (SELECT * FROM regions WHERE active = $1) AS r",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_Join_SubQuery_DefaultAlias(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users JOIN (SELECT * FROM posts WHERE published = ?) AS sub ON users.id = sub.user_id",
			dialects.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM posts WHERE published = $1) AS sub ON users.id = sub.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{true}, bindings)
...
func Test_Join_SubQuery_With_Alias(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users JOIN (SELECT * FROM posts WHERE published = ?) AS p ON users.id = p.user_id",
			dialects.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM posts WHERE published = $1) AS p ON users.id = p.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{true}, bindings)
...
func Test_LeftJoin_SubQuery(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users LEFT JOIN (SELECT * FROM comments WHERE active = ?) AS c ON users.id = c.user_id",
			dialects.DriverPostgres: "SELECT * FROM users LEFT JOIN (SELECT * FROM comments WHERE active = $1) AS c ON users.id = c.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{true}, bindings)
...
func Test_RightJoin_SubQuery(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users RIGHT JOIN (SELECT * FROM orders WHERE status = ?) AS o ON users.id = o.user_id",
			dialects.DriverPostgres: "SELECT * FROM users RIGHT JOIN (SELECT * FROM orders WHERE status = $1) AS o ON users.id = o.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_FullJoinExpr(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "",
			dialects.DriverPostgres: "SELECT * FROM users FULL JOIN (SELECT * FROM stats WHERE active = $1) AS s ON users.id = s.user_id",
		}
		expectedErr := map[dialects.Driver]error{
			dialects.DriverMySQL:    xqbErr.ErrUnsupportedFeature, // MySQL does not support FULL JOIN
			dialects.DriverPostgres: nil,
...
func Test_JoinExpr_With_Expression_Condition(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users JOIN (SELECT * FROM payments WHERE confirmed = ?) AS p ON users.id = p.user_id AND p.status = ?",
			dialects.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM payments WHERE confirmed = $1) AS p ON users.id = p.user_id AND p.status = $2",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_CrossJoinSub_FallbackAlias(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		sub := xqb.Table("plans").SetDialect(dialect).Where("expired", "=", false)
		qb := xqb.Table("users").SetDialect(dialect).CrossJoinSub(sub, "")
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users CROSS JOIN (SELECT * FROM plans WHERE expired = ?) AS sub",
			dialects.DriverPostgres: "SELECT * FROM users CROSS JOIN (SELECT * FROM plans WHERE expired = $1) AS sub",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{false}, bindings)
...
func Test_Multiple_Joins_Mixed_Types(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL: "SELECT * FROM users JOIN addresses ON users.id = addresses.user_id AND addresses.city = ?" +
			...
			dialects.DriverPostgres: "SELECT * FROM users JOIN addresses ON users.id = addresses.user_id AND addresses.city = $1" +
...
func Test_Join_With_SubQuery_That_Has_Join(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		innerSub := xqb.Table("payments").SetDialect(dialect).Where("amount", ">", 500)
		...
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL: "SELECT * FROM users JOIN (SELECT * FROM orders JOIN (SELECT * FROM payments WHERE amount > ?) AS pay" +
				" ON orders.payment_id = pay.id WHERE orders.status = ?) AS o ON users.id = o.user_id",
			dialects.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM orders JOIN (SELECT * FROM payments WHERE amount > $1) AS pay" +
...
func Test_CrossJoin_Combined_With_Other_Joins(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "SELECT * FROM users JOIN posts ON users.id = posts.user_id CROSS JOIN countries",
			dialects.DriverPostgres: "SELECT * FROM users JOIN posts ON users.id = posts.user_id CROSS JOIN countries",
		}
...
func Test_Stores_With_Orders_SubQuery(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL: "SELECT managers.fullname, managers.email, stores.id, order_stats.total_orders, locations.city location_city, locations.zip_code location_zip, managers.id manager_id " +
				"FROM stores " +
				...
				"LIMIT 5",
			dialects.DriverPostgres: "SELECT managers.fullname, managers.email, stores.id, order_stats.total_orders, locations.city location_city, locations.zip_code location_zip, managers.id manager_id " +
				"FROM stores " +
