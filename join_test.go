...
func Test_Join_String_Table(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		qb := xqb.Table("users").SetDialect(dialect).Join("posts", "users.id = posts.user_id")
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users JOIN posts ON users.id = posts.user_id",
			grammar.DriverPostgres: "SELECT * FROM users JOIN posts ON users.id = posts.user_id",
		}
...
func Test_Join_With_Bindings(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users JOIN posts ON users.id = posts.user_id AND posts.status = ?",
			grammar.DriverPostgres: "SELECT * FROM users JOIN posts ON users.id = posts.user_id AND posts.status = ?",
		}
		assert.Equal(t, expectedSql[dialect], sql)
...
func Test_LeftJoin(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users LEFT JOIN comments ON users.id = comments.user_id",
			grammar.DriverPostgres: "SELECT * FROM users LEFT JOIN comments ON users.id = comments.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Empty(t, bindings)
...
func Test_RightJoin(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users RIGHT JOIN logins ON users.id = logins.user_id",
			grammar.DriverPostgres: "SELECT * FROM users RIGHT JOIN logins ON users.id = logins.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Empty(t, bindings)
...
func Test_CrossJoin(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users CROSS JOIN roles",
			grammar.DriverPostgres: "SELECT * FROM users CROSS JOIN roles",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Empty(t, bindings)
...
func Test_CrossJoinSub(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users CROSS JOIN (SELECT * FROM plans WHERE expired = ?) AS p",
			grammar.DriverPostgres: "SELECT * FROM users CROSS JOIN (SELECT * FROM plans WHERE expired = ?) AS p",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_CrossJoin_With_Expr(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users CROSS JOIN (SELECT * FROM regions WHERE active = ?) AS r",
			grammar.DriverPostgres: "SELECT * FROM users CROSS JOIN (SELECT * FROM regions WHERE active = ?) AS r",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_Join_SubQuery_DefaultAlias(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users JOIN (SELECT * FROM posts WHERE published = ?) AS sub ON users.id = sub.user_id",
			grammar.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM posts WHERE published = ?) AS sub ON users.id = sub.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{true}, bindings)
...
func Test_Join_SubQuery_With_Alias(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users JOIN (SELECT * FROM posts WHERE published = ?) AS p ON users.id = p.user_id",
			grammar.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM posts WHERE published = ?) AS p ON users.id = p.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{true}, bindings)
...
func Test_LeftJoin_SubQuery(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users LEFT JOIN (SELECT * FROM comments WHERE active = ?) AS c ON users.id = c.user_id",
			grammar.DriverPostgres: "SELECT * FROM users LEFT JOIN (SELECT * FROM comments WHERE active = ?) AS c ON users.id = c.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{true}, bindings)
...
func Test_RightJoin_SubQuery(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users RIGHT JOIN (SELECT * FROM orders WHERE status = ?) AS o ON users.id = o.user_id",
			grammar.DriverPostgres: "SELECT * FROM users RIGHT JOIN (SELECT * FROM orders WHERE status = ?) AS o ON users.id = o.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_JoinExpr_With_Expression_Condition(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users JOIN (SELECT * FROM payments WHERE confirmed = ?) AS p ON users.id = p.user_id AND p.status = ?",
			grammar.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM payments WHERE confirmed = ?) AS p ON users.id = p.user_id AND p.status = ?",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_CrossJoinSub_FallbackAlias(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users CROSS JOIN (SELECT * FROM plans WHERE expired = ?) AS sub",
			grammar.DriverPostgres: "SELECT * FROM users CROSS JOIN (SELECT * FROM plans WHERE expired = ?) AS sub",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{false}, bindings)
...
func Test_Multiple_Joins_Mixed_Types(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		sql, bindings, err := qb.ToSQL()
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL: "SELECT * FROM users JOIN addresses ON users.id = addresses.user_id AND addresses.city = ?" +
			...
			grammar.DriverPostgres: "SELECT * FROM users JOIN addresses ON users.id = addresses.user_id AND addresses.city = ?" +
				" LEFT JOIN (SELECT * FROM orders WHERE status = ?) AS o ON users.id = o.user_id" +
				" RIGHT JOIN (SELECT * FROM invoices WHERE paid = ?) AS inv ON users.id = inv.user_id AND inv.total > ?",
		}
...
func Test_Join_With_SubQuery_That_Has_Join(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL: "SELECT * FROM users JOIN (SELECT * FROM orders JOIN (SELECT * FROM payments WHERE amount > ?) AS pay" +
				" ON orders.payment_id = pay.id WHERE orders.status = ?) AS o ON users.id = o.user_id",
			grammar.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM orders JOIN (SELECT * FROM payments WHERE amount > ?) AS pay" +
				" ON orders.payment_id = pay.id WHERE orders.status = ?) AS o ON users.id = o.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_CrossJoin_Combined_With_Other_Joins(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL:    "SELECT * FROM users JOIN posts ON users.id = posts.user_id CROSS JOIN countries",
			grammar.DriverPostgres: "SELECT * FROM users JOIN posts ON users.id = posts.user_id CROSS JOIN countries",
		}
...
func Test_Stores_With_Orders_SubQuery(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect grammar.Driver) {
		...
		expectedSql := map[grammar.Driver]string{
			grammar.DriverMySQL: "SELECT managers.fullname, managers.email, stores.id, order_stats.total_orders, locations.city location_city, locations.zip_code location_zip, managers.id manager_id " +
				"FROM stores " +
				...
				"LIMIT 5",
			grammar.DriverPostgres: "SELECT managers.fullname, managers.email, stores.id, order_stats.total_orders, locations.city location_city, locations.zip_code location_zip, managers.id manager_id " +
				"FROM stores " +
				"LEFT JOIN (SELECT store_id, COUNT(*) as total_orders FROM orders WHERE cancelled_at IS NULL AND confirmed_at IS NOT NULL AND status != ? GROUP BY store_id) AS order_stats ON stores.id = order_stats.store_id " +
				"JOIN managers ON stores.manager_id = managers.id " +
				"JOIN locations ON stores.location_id = locations.id " +
