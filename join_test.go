...
func Test_Join_With_Bindings(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users JOIN posts ON users.id = posts.user_id AND posts.status = ?",
		}
		assert.Equal(t, expectedSql[dialect], sql)
...
func Test_CrossJoinSub(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users CROSS JOIN (SELECT * FROM plans WHERE expired = ?) AS p",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_CrossJoin_With_Expr(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users CROSS JOIN (SELECT * FROM regions WHERE active = ?) AS r",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_Join_SubQuery_DefaultAlias(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM posts WHERE published = ?) AS sub ON users.id = sub.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{true}, bindings)
...
func Test_Join_SubQuery_With_Alias(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM posts WHERE published = ?) AS p ON users.id = p.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{true}, bindings)
...
func Test_LeftJoin_SubQuery(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users LEFT JOIN (SELECT * FROM comments WHERE active = ?) AS c ON users.id = c.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{true}, bindings)
...
func Test_RightJoin_SubQuery(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users RIGHT JOIN (SELECT * FROM orders WHERE status = ?) AS o ON users.id = o.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_FullJoinExpr(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			dialects.DriverMySQL:    "",
			dialects.DriverPostgres: "SELECT * FROM users FULL JOIN (SELECT * FROM stats WHERE active = ?) AS s ON users.id = s.user_id",
		}
		expectedErr := map[dialects.Driver]error{
			dialects.DriverMySQL:    xqbErr.ErrUnsupportedFeature, // MySQL does not support FULL JOIN
			dialects.DriverPostgres: nil,
...
func Test_JoinExpr_With_Expression_Condition(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM payments WHERE confirmed = ?) AS p ON users.id = p.user_id AND p.status = ?",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
...
func Test_CrossJoinSub_FallbackAlias(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users CROSS JOIN (SELECT * FROM plans WHERE expired = ?) AS sub",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
		assert.Equal(t, []any{false}, bindings)
...
func Test_Multiple_Joins_Mixed_Types(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users JOIN addresses ON users.id = addresses.user_id AND addresses.city = ?" +
				" LEFT JOIN (SELECT * FROM orders WHERE status = ?) AS o ON users.id = o.user_id" +
				" RIGHT JOIN (SELECT * FROM invoices WHERE paid = ?) AS inv ON users.id = inv.user_id AND inv.total > ?",
		}
...
func Test_Join_With_SubQuery_That_Has_Join(t *testing.T) {
	forEachDialect(t, func(t *testing.T, dialect dialects.Driver) {
		...
		expectedSql := map[dialects.Driver]string{
			...
			dialects.DriverPostgres: "SELECT * FROM users JOIN (SELECT * FROM orders JOIN (SELECT * FROM payments WHERE amount > ?) AS pay" +
				" ON orders.payment_id = pay.id WHERE orders.status = ?) AS o ON users.id = o.user_id",
		}
		assert.Equal(t, expectedSql[dialect], sql)
		assert.NoError(t, err)
